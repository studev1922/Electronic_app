...
public class Random {
     ...
     * @return longtime
     */
    public static long randomDate(Date start, Date end) {
        if (start == null) {
            start = new Date(0);
        }
        if (end == null) {
        ...
        long origin = start.getTime();
        long bound = end.getTime();
    ...
    public static String randomDate(Date start, Date end, String pattern) {
        long longTime = Random.randomDate(start, end);
        return new SimpleDateFormat(pattern).format(longTime);
    }
}
